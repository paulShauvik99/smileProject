...
export default function RequestDashboard() {
    ...
    const placeRequest = async (patDet) =>{ 
        ...
        try {
            const res = await axios.post('http://192.168.56.1:8000/recipient/request_blood/',formData);
            console.log(res)
            Swal.fire({
    ...
    const loadAPI = async () =>{
        setLoadingPage(true)
        try {
            const res = await axios.get('http://192.168.56.1:8000/recipient/get_recipient_records/')
            console.log(res)       
    ...
    const logout = () => {
        try{
            axios.get('http://192.168.56.1:8000/donor/logout/').then((res)=>{
                setLoadingPage(true)
                localStorage.removeItem('check')
                Swal.fire({
