...
// OTP Value
    ...
    const sendOtp = async () =>{
        ...
        if(donorInfo.firstName === '' || donorInfo.lastName === '' || donorInfo.dob === '' || donorInfo.email === '' || donorInfo.phoneNumber === '' || donorInfo.address === '' || donorInfo.bloodGroup === ''){
            ...
            timer()
            try{
                const res =  await axios.post('http://192.168.56.147474747471:8000/donor/send_otp/', JSON.stringify(phoneNumber))
    ...
    const verifyOtp = async () =>{
        ...
        try {
            const res = await axios.post('http://192.168.56.147474747471:8000/donor/register/',JSON.stringify(donorDet))
            if('success' in res.data){
                const now = new Date().getTime()
                        let check = {
